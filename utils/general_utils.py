...
def Compile(code_input, code_output, file_path, editor, is_on):
    '''
    ...
    if file_path == '':
        MultiThread(Speak,'Please save your code first')
        return
    filetype = file_path.split('.')
    if 'cpp' in filetype[1]:   
        ...
        MultiThread(Speak, "Can't compile .txt files")                
        code_output.insert('1.0', "Can't compile .txt files")
        return
    elif 'py' in filetype[1]:
        MultiThread(Speak, "Python is an interpreted language and not a compiled one, Click on run to run the program")                
    ...
    #Most of the times error in the later parts are associated with the initial errors so the user is told the error in the initial part so that user doesn't has to listen the complete error log
    # print(error)
    if error is not b'' and is_on == True:
        gist = error.split('\n')
        if len(gist)<=4:           
            MultiThread(Speak, error)
        else:
            gist = gist[0]+"\n" + gist[1] + "\n" + gist[2] + "\n" + gist[3]
            MultiThread(Speak, gist)
    return error

def Run(code_input, code_output, file_path, editor, is_on):
    '''
    ...
    filetype = file_path.split('.')
    if 'cpp' in filetype[1]:
        Compile(code_input, code_output, file_path, editor, is_on)
        ...
        obj = obj.split('.')[0]
        print(obj)
        p = subprocess.Popen(['java', '-cp', './files;',obj], stdout=subprocess.PIPE, stdin = subprocess.PIPE, stderr=subprocess.PIPE, shell = True)
        code = editor.get('1.0', END)
        if "Scanner" in code:
            inp = inp.encode('ascii')   
            p.stdin.write(inp)
    ...
    if output is not b'' and is_on == True: 
        output = output.decode('utf-8')
        MultiThread(Speak, output)